...
// Serialized tests because they modify system network state
var _ = Describe("Controller Run", Serial, func() {
	var (
		logBuf *bytes.Buffer
		log    logr.Logger
	...
	BeforeEach(func() {
		...
		log, _ = logger.NewZapLogger("debug", "text", zap.StacktraceLevel(zapcore.PanicLevel), zap.WriteTo(logBuf))
	...
	AfterEach(func() {
		...
		// clean up created tap + bonding devices
	...
	It("triggers a timeout when no udp packets arrive", func() {
		// wait for longer than the controller's read deadline
		time.Sleep(constants.TunnelControllerUpdateTimeout + 1*time.Second)

		// eventually the log should contain a timeout error
		Eventually(func() string {
			return logBuf.String()
		}, "10s", "500ms").Should(ContainSubstring("i/o timeout"))
	})
